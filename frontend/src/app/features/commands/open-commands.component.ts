...
@Component({
  ...
  template: `
    ...
    <mat-form-field appearance="outline" style="max-width: 300px;">
      <mat-label>Buscar por número, mesa, cliente ou operador</mat-label>
    ...
    <div class="grid">
      @for (comanda of comandasFiltradas; track comanda.id) {
        <mat-card class="card">
          ...
          @if (comanda.numeroMesa || comanda.nomeCliente) {
          ...
          <p class="total">
            Total: R$ {{ comanda.valorTotal | number: "1.2-2" }}
          </p>
          <p class="usuario">Usuário: {{ comanda.abertaPor }}</p>
          <button mat-flat-button color="primary" (click)="abrir(comanda)">
            ABRIR
          </button>
        </mat-card>
      } @empty {
...
export class OpenCommandsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.carregar();
    this.subs.add(
      this.realtime.comandaAtualizada$.subscribe(() => this.carregar()),
    );
    this.subs.add(
      this.realtime.comandaAberta$.subscribe(() => this.carregar()),
    );
  }
  ...
  get comandasFiltradas(): ComandaResumo[] {
    ...
    return this.comandas.filter(
      (c) =>
        ...
        c.abertaPor.toLowerCase().includes(termo) ||
        c.numeroMesa?.toString().includes(termo) ||
  ...
  abrir(comanda: ComandaResumo): void {
    ...
    this.router.navigate(["/pdv"], {
      queryParams: { comanda: comanda.numero },
    });
  }
}
